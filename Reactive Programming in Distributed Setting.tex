...
The Observer pattern \cite{ojpg} is commonly used to implement distributed event handling. However, the pattern is also highly criticized due to the overhead it creates in terms of boilerplate code that has to be added to the application. Its implementation also involves pollution of the interface of the classes because of the methods added in order to handle the message passing. It also causes inversion of control and can potentially cause memory leaks. The Observer pattern is briefly described by the UML diagram in Figure 1.
...
\textbf{Reactive languages} also support the typical for the imperative style of programming events. For example, in REScala \cite{rescala} there is built-in event notifying when a change to a reactive value has been made. There are also a lot of conversion functions that create signals from events.
